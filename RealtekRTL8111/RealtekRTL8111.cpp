...
void RTL8111::ShowMacAddr(const IOEthernetAddress HWAddr)
{
    size_t shownlen = 0;
    
    if (HWAddr.bytes  == NULL)
    {
        return;
    }
    
    while (shownlen < kIOEthernetAddressSize)
    {
        IOLog("%.2X", HWAddr.bytes[shownlen]);
        
        ++shownlen;
        
        if (shownlen < kIOEthernetAddressSize)
        {
            IOLog(":");
        }
    }
...
IOEthernetAddress RTL8111::GetOrigMACAddr(void)
{
    return origMacAddr;
}

bool RTL8111::MergeDictionaryIntoProvider(IOService * provider, OSDictionary * dictionaryToMerge)
{
...
bool RTL8111::MergeDictionaryIntoDictionary(OSDictionary * parentSourceDictionary,  OSDictionary * parentTargetDictionary)
{
...
void RTL8111::SetIsCustomMACAddr(Boolean used)
{
    OSData *newMACAddr = NULL;
    const OSString *MACAddrString[] = { OSString::withCString("IOMACAddress"), NULL };
    OSDictionary *newMACDict = NULL;
    UInt8 *data;
    ...
    if (used == false)
    {
        newMACAddr = OSData::withBytesNoCopy(&origMacAddr.bytes, kIOEthernetAddressSize);
        bcopy(&origMacAddr.bytes, &emuMacAddr.bytes, kIOEthernetAddressSize);
    } else {
        newMACAddr = OSData::withBytesNoCopy(&currMacAddr.bytes, kIOEthernetAddressSize);
        bcopy(&currMacAddr.bytes, &emuMacAddr.bytes, kIOEthernetAddressSize);
    }
    
    const OSObject *newMACAddrObj[] = { OSDynamicCast(OSObject, newMACAddr), NULL };
    
    newMACDict = OSDictionary::withObjects(newMACAddrObj, MACAddrString, 1);
    data = (UInt8 *)newMACAddr->getBytesNoCopy();
    
    IOLog("Ethernet [RealtekRTL8111]: Set new MAC address: %.2X:%.2X:%.2X:%.2X:%.2X:%.2X\n", data[0], data[1], data[2], data[3], data[4], data[5]);
    
    MergeDictionaryIntoProvider(cardprovider, newMACDict);
    MergeDictionaryIntoProvider(cardprovider->getClient(), newMACDict);
    
    newMACDict->release();
    newMACAddr->release();
    MACAddrString[0]->release();
    
    newMACAddrObj[0] = NULL;
    MACAddrString[0] = NULL;
    newMACAddr = NULL;
    newMACDict = NULL;
}

IOReturn RTL8111::newUserClient(task_t owningTask, void * securityID, UInt32 type, IOUserClient ** handler)
{
    IOReturn ioReturn = kIOReturnSuccess;
    RTL8111Client *client = NULL;
    
    if (mClientCount > MAXUSERS)
    {
        IOLog("Ethernet [RealtekRTL8111]: Client already created, not deleted\n");
        
        return(kIOReturnError);
    }
    
    client = (RTL8111Client *)RTL8111Client::withTask(owningTask);
    if (client == NULL)
    {
        ioReturn = kIOReturnNoResources;
        
        IOLog("Ethernet [RealtekRTL8111]: newUserClient: Can't create user client\n");
    }
    ...
    if (ioReturn == kIOReturnSuccess)
    {
        ...
        if (client->start(this) == false) {
            ...
            IOLog("Ethernet [RealtekRTL8111]: newUserClient: Can't start user client\n");
        }
    }
    
    if ((ioReturn != kIOReturnSuccess) && (client != NULL))
    {
        IOLog("Ethernet [RealtekRTL8111]: newUserClient error\n");
        client->detach(this);
        client->release();
    } else {
    ...
    DebugLog("RTL8111: newUserClient() client = %p\n", mClientPtr[mClientCount]);
    
    return (ioReturn);
}

void RTL8111::setErr( bool set )
{
    mErrFlag = set;
}

void RTL8111::closeChild(RTL8111Client *ptr)
{
    UInt8 i, idx;
    idx = 0;
    ...
    if (mClientCount == 0)
    {
        IOLog("Ethernet [RealtekRTL8111]: No clients available to close");
        return;
    }
    
#ifdef  DEBUG
    IOLog("Ethernet [RealtekRTL8111]: Closing: %p\n",ptr);
    
    for(i=0;i<mClientCount;i++)
    {
        IOLog("Ethernet [RealtekRTL8111]: userclient ref: %d %p\n", i, mClientPtr[i]);
    }
#endif
...
bool RTL8111::start(IOService *provider)
{
    ...
    newMACAddr = OSDynamicCast(OSData, getProperty(kMACAddrName));
    
    emuMacAddrUsed = false;
    
    if (newMACAddr)
    {
        bcopy(newMACAddr->getBytesNoCopy(), &emuMacAddr.bytes, ((newMACAddr->getLength() <= 6) ? newMACAddr->getLength() : 6));
        
        if (strlen((char *)&emuMacAddr) == 6)
        {
            memset(&emuMacAddrUsed, true, 1);
        }
        
        IOLog("Ethernet [RealtekRTL8111]: Initialized emulated MAC address = ");
        ShowMacAddr(emuMacAddr);
    }
...
void RTL8111::stop(IOService *provider)
{
    ...
    //DebugLog("Driver unloaded, total debug count recorded=%lld", debugcount);
...
IOReturn RTL8111::runAction(UInt32 action, UInt32 *outSize, void **outData, void *extraArg)
{
    DebugLog("runAction() ===>\n");
